...
encoding of exactly 32 bytes generated by a cryptographically secure random source. The harness can enforce
the encoding and reject obvious low-diversity values, but cannot prove how a supplied key was generated.
Evidence stores only the portal host, role, nullable documented portal build,
scope hash, key-format assertion, and HMAC-SHA256 over the canonical
`(host, role, principal_id)` tuple. The host is the stable portal name; the
fingerprint distinguishes the selected role/principal without persisting the
principal ID or key. Unknown build remains null and cannot authorize a
build-scoped endpoint profile. Low-diversity or low-entropy 32-byte values are
rejected rather than passing on length alone.
...
genesis iff `sequence == 0`, immutable lineage, previous-record links, a plan-bound
per-correlation lifecycle state machine, and a
...
three real model mutations/retries and records distinct independent pre/post hashes. Stable oracle files
bind expected and actual result hashes and parent evidence binds their canonical SHA-256. Draft timing
