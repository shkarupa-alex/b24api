...
class CompletionRecorder:
    ...
    def settled(self, outcome: CommandSettlement) -> None:
        ...
        self.gate.emit(PageCommandOutcome(
            operation_id=self.gate.operation_id, sequence=self._take(), binding_id=0,
            page_id=page_id, outcome=outcome,
        ))
    ...
    def validated(self, identities: Sequence[IdentityValue], row_count: int) -> None:
        ...
        self.gate.emit(PageValidated(
            operation_id=self.gate.operation_id, sequence=self._take(), binding_id=0,
            page_id=page_id, identity_digest=digest, row_count=row_count,
        ))
        if row_count == 0:
            self.delivered()
            self.acknowledged()
    ...
    def acknowledged(self) -> None:
        ...
        page_id = self._require_page()
        self.gate.emit(PageAcknowledged(
            operation_id=self.gate.operation_id, sequence=self._take(), binding_id=0, page_id=page_id,
        ))
        self._current = None

    def terminal(self, closure: BindingClosure, stream: StreamClosure) -> None:
        """Settle binding and producer before owned-resource cleanup."""
        self.gate.emit(BindingTerminal(
            operation_id=self.gate.operation_id, sequence=self._take(), binding_id=0, closure=closure,
        ))
        self.gate.emit(StreamTerminal(
            operation_id=self.gate.operation_id, sequence=self._take(), closure=stream,
        ))

    def terminal_from_plan(self, plan: ListPlan, state: KernelState, *, caller_stopped: bool) -> None:
        """Preserve the driver's qualified closure distinction in gate evidence."""
        closure = (
            BindingClosure.CALLER_STOP if caller_stopped
            else BindingClosure.BOUNDARY_SEEN if isinstance(plan, KeysetPlan) and plan.boundary
            else BindingClosure.SOURCE_EMPTY if state is KernelState.COMPLETED
