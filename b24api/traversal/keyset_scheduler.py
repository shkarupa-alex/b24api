...
from b24api.contracts.keyset_execution import (
    AutoKeysetExecution, ClosureWitness, KeysetExecutionKind, KeysetPageCompletion, KeysetPhase, KeysetSelectionReason,
    PartitionedKeysetExecution, RangeKeysetExecution, StableIntegerKeysetContract, TotalHintMode,
)
...
from b24api.traversal.keyset_capability import (
    CapabilityCommand, analyze_boundary, anchor_commands, canary_commands, compact_anchor_receipts,
...
class KeysetFastScheduler:
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        self.executor, self.request, self.identity, self.keyset = executor, request, identity, keyset
        ...
        self._anchors_precharged = False
        ...
        self._last_wave_responses: dict[str, Response] = {}
    @property
    ...
    def _lane_plan(self, lane: LaneState, *, phase: KeysetPhase, request: Request, reserve: int | None = None,
                   single: bool = False) -> LaneCommandPlan:
        return LaneCommandPlan(lane.spec.ordinal, self._next_command_id(phase), phase, request,
                               reserve or self.effective_page_cap, single)
    def _controls(self, *, direction: str, lower: int | None = None, upper: int | None = None, limit: int | None = None,
                  advisory_start: bool = False) -> Request:
        return keyset_step.bounded_keyset_request(self.request, keyset=self.keyset, identity=self.identity,
                                                  direction=direction, lower=lower, upper=upper, limit=limit,
                                                  advisory_start=advisory_start)
    def _record(  # noqa: PLR0913
        self, plan: LaneCommandPlan, *, index: int | None, selected: int, admitted: int,
        outcome: PageOutcome = PageOutcome.COMMITTED, rejection: PageRejectionCode | None = None,
        violation: Violation | None = None, response: Response | None = None,
        witness: ClosureWitness | None = None, dispatch: PageDispatch = PageDispatch.BATCH,
    ) -> None:
    ...
    async def _wave(self, plans: tuple[LaneCommandPlan, ...]) -> tuple[LaneReceipt, ...]:  # noqa: C901, PLR0912
        ...
        try:
            ...
            self._last_wave_responses = {plan.command_id: outcome.response
            ...
            for index, (plan, outcome, reservation) in enumerate(zip(plans, outcomes, reservations, strict=True)):
                ...
                receipt = validate_lane_receipt(
                    plan=plan, lane=lane, outcome=outcome, identity=self.identity,
                    ...
                    completion=self._completion, selector=self.selector)
                if isinstance(receipt, ReceiptRejection):
                    ...
                    self._record(plan, index=index, selected=0, admitted=0, outcome=page_outcome,
            ...
            if failed:
                ...
                for index, plan in enumerate(plans):
                    if plan.command_id in successful:
                        self._record(plan, index=index, selected=len(successful[plan.command_id].rows), admitted=0,
            ...
            for index, (plan, receipt) in enumerate(zip(plans, receipts, strict=True)):
                ...
                self._record(plan, index=index, selected=len(receipt.rows), admitted=0,
    ...
    async def _chunked_waves(self, plans: tuple[LaneCommandPlan, ...]) -> tuple[LaneReceipt, ...]:
        receipts: list[LaneReceipt] = []
        remaining = plans
        while remaining:
            wave = fit_wave(
            ...
            receipts.extend(await self._wave(wave))
    ...
    async def _chunked_anchor_waves(self, plans: tuple[LaneCommandPlan, ...]) -> tuple[tuple[LaneReceipt, ...], int]:
        compact: list[LaneReceipt] = []
        precharged = 0
        remaining = plans
        while remaining:
            wave = fit_wave(
            ...
            if not wave:
                raise RuntimeError("fast keyset anchor wave has no available row capacity")
            wave_compact, discarded, retained = compact_anchor_receipts(await self._wave(wave))
            compact.extend(wave_compact)
            self.admission.record_raw(discarded)
            self.admission.record_discarded(discarded)
            precharged += retained
            await self._adjust_buffer(retained)
            remaining = remaining[len(wave) :]
        return tuple(compact), precharged
    async def _partition_planning(self, asc: LaneReceipt, desc: LaneReceipt, target: int) -> tuple[int, ...]:
        ...
        if len(co_scheduled) == len(planning):
            ...
            anchor_receipts, precharged = await self._chunked_anchor_waves(anchors) if anchors else ((), 0)
        self._validate_canaries(canary_receipts, expected)
        if len(co_scheduled) == len(planning):
            return self._consume_anchors(anchor_receipts, lo=max(asc.identities), hi=min(desc.identities))
        result = self._consume_anchors(anchor_receipts, lo=max(asc.identities), hi=min(desc.identities))
        await self._adjust_buffer(-(precharged - self._anchor_count))
        self._anchors_precharged = True
        return result
    ...
    async def plan_barrier(self) -> PlanOutcome:  # noqa: C901, PLR0912
        ...
        advisory = (isinstance(self.execution, AutoKeysetExecution)
        ...
        asc_response = self._last_wave_responses.get(asc.command_id)
        desc_response = self._last_wave_responses.get(desc.command_id)
        analysis = analyze_boundary(
            asc, desc,
            ascending_total=asc_response.total if asc_response is not None else None,
            descending_total=desc_response.total if desc_response is not None else None,
            advisory=advisory,
        )
        ...
        if isinstance(self.execution, AutoKeysetExecution):
            selector_capacity = (self.batch_capacity
                                 if available_rows >= self.effective_page_cap + self.execution.target_lanes else 0)
            inputs = SelectorInputs(
                analysis.facts, self.effective_page_cap, selector_capacity, self.execution.target_lanes,
                self.execution.max_range_waves, self.execution.range_window_width, self._completion,
            ...
            if (bounded and self._completion is KeysetPageCompletion.SHORT_PAGE_EXHAUSTS
            ...
            if (self._completion is KeysetPageCompletion.SHORT_PAGE_EXHAUSTS
                    and (len(asc.rows) != self.effective_page_cap or len(desc.rows) != self.effective_page_cap)):
                raise PaginationError("boundary pages did not establish page-cap agreement")
        ...
        plan = FastKeysetPlan(selected, tuple(lane.spec for lane in self._lanes), self.page_size,
                              self.effective_page_cap, self.batch_capacity, self._completion, self.identity,
    ...
    async def _configure_selected(self, asc: LaneReceipt, desc: LaneReceipt) -> None:
        ...
        geometry = selected_lane_geometry(
            selected=self._selected, execution=self.execution, keyset=self.keyset, completion=self._completion,
        ...
        if self._selected is KeysetExecutionKind.PARTITIONED and self._anchor_count and not self._anchors_precharged:
            await self._adjust_buffer(self._anchor_count)
        ...
        for lane in open_lanes[: len(plans)]:
            ...
            if lane.spec.descending:
                witness = descending_closure_witness(
                    lane, receipt.identities, completion=self._completion, page_cap=self.effective_page_cap)
